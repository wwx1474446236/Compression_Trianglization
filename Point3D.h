...
template<typename _Tp> inline CPoint3D<_Tp>::CPoint3D() :x(0), y(0), z(0) {}

template<typename _Tp> inline CPoint3D<_Tp>::CPoint3D(_Tp _x, _Tp _y, _Tp _z) { x = _x; y = _y; z = _z; }

template<typename _Tp> inline CPoint3D<_Tp>::CPoint3D(const CPoint3D& pt) : x(pt.x), y(pt.y), z(pt.z) {}
...
{
	return CPoint3D<_Tp>(y*pt.z - z * pt.y, z*pt.x - x * pt.z, x*pt.y - y * pt.x);
}
