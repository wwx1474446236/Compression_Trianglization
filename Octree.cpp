...
namespace cv
{
	...
	bool COctree::ReadFile(char filePath[], vector<CPoint3f>& tempPoint)
	{
		...
		{
			CPoint3f NewPoint;
			fscanf(file, "%f %f %f", &NewPoint.x, &NewPoint.y, &NewPoint.z);
	...
	bool COctree::WriteFile(char filePath[]) // 将压缩后的文件以.pcd格式储存
	{
		...
		if (fOutput)
		{
			...
			{
				fOutput << m_simplifypoints[i].x << ' ' << m_simplifypoints[i].y << ' ' << m_simplifypoints[i].z << '\n';
			}
		}
		else
			return false;
	...
	void COctree::outArray(float *X, float *Y, float *Z) {
		if (m_simplifypoints.empty())cout << "cloud point is empty!" << endl;
		int size = m_simplifypoints.size();
		for (int i = 0; i < size; ++i)
		{
			X[i] = m_simplifypoints[i].x;
			Y[i] = m_simplifypoints[i].y;
			Z[i] = m_simplifypoints[i].z;
		}
	}

	int COctree::getSize() {
	return m_simplifypoints.size();
	}
